...
const CategoryPageContent = ({
    ...
    () => [
      {
        ...
        header: "Category",
        cell: () => (
          <span className="px-2 py-1 rounded-md bg-gray-100 text-gray-800 font-medium">
      ...
      ...(data?.events[0]
        ? Object.keys(data.events[0].feilds as object).map((field) => ({
            accessorFn: (row: Event) =>
              (row.feilds as Record<string, any>)[field],
            header: field,
            cell: ({ row }: { row: Row<Event> }) =>
              (row.original.feilds as Record<string, any>)[field] || "-",
          }))
        : []),
      {
  ...
  const numericFieldSums = useMemo(() => {
    ...
    data.events.forEach((event) => {
      ...
      Object.entries(event.feilds as object).forEach(([field, value]) => {
